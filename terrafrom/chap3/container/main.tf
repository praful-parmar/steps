...
resource "docker_container" "app_container" {
  ...
  count = var.count_in
  name  = join("-", [var.name_in, terraform.workspace, random_string.randomstring[count.index].result])
  image = var.image_in
  ports {
...
module "volume" {
  ...
  volume_name  = "${var.name_in}-${terraform.workspace}-${random_string.randomstring[count.index].result}-volume"
}